...
class Bot:
    ...
    def _inbound_stanzas(self, obj):
        ...
        print("Stanza arrived:\n{}".format(obj.to_str()))

        if obj.kind == 'message':

            cmd_line = org.wayround.utils.shlex.split(obj.body.find('{jabber:client}body').text.splitlines()[0])
            ...
            ret_stanza = org.wayround.xmpp.core.Stanza(
                ...
                kind='message',
                typ='chat',
                body='<body>Not authorized</body>'
                )

            if (org.wayround.xmpp.core.jid_from_string(obj.jid_from).bare()
                == self._site.admin_jid):
